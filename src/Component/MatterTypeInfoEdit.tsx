...
function MatterTypeInfoEdit(props: any) {
    const { rowData } = props;
    const [localStateContent, setlocalStateContent] = useState<any>({
        VehicleModelId: rowData?.VehicleModelId ?? "",
        VehicleVinNo: rowData?.VehicleVinNo ?? "",
        PurchaseDate: rowData?.PurchaseDate ?? "",
        VehiclePurchaseTypeId: rowData?.VehiclePurchaseTypeId ?? "",
        OtherVehicleSalesInfo: rowData?.OtherVehicleSalesInfo ?? "",
        RISCSalesLeaseContract: rowData?.RiscsalesLeaseContract ?? false,
        CABuyBackRequestFlag: rowData?.CabuyBackRequestFlag ?? false,
    });

    useEffect(() => {
        localStorage.setItem("localStateMatterTypeContentEdit", JSON.stringify(localStateContent));
    }, [localStateContent]);
...
export default memo(MatterTypeInfoEdit)