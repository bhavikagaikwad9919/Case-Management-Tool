...
function CounselInfo() {
    ...
    const editData = async (updatedData:any) => {
        try {
          const response = await fetch(`${process.env.REACT_APP_BASE_URL}/Counsel/${updatedData.id}`, {
            method: 'PUT',
            headers: { 
    ...
    const editCommandClick = (args:any) => {
        console.log("helloapi")
        const CounselLawFirm = args.commandColumn?.type;
        const rowData = args.rowData;
        if (CounselLawFirm === 'edit') {
          editData(rowData);
        } else if (CounselLawFirm === 'add') {
          const newData = { };
          addData(newData);
        }
    }
    return (
        <div className="control-pane">
            <div className="control-section">
                ...
                >
                    <ColumnsDirective>
                        <ColumnDirective field="Id" headerText="Id" width="125" textAlign="Center" isPrimaryKey={true} visible={false} />
                        <ColumnDirective field="CounselLawFirm" headerText="Counsel Law Firm" width="125" textAlign="Center" />
                        <ColumnDirective field="CounselEmail" headerText="Counsel Email" width="125" />
                        <ColumnDirective field="CounselDomain" headerText="Counsel Domain" width="180" />
                        <ColumnDirective field="IsActive" headerText="Status" width="100" type="boolean" editType="booleanedit" displayAsCheckBox={true} visible={false} />
                        <ColumnDirective field="Created" headerText="Created" width="160" format={formatAmPm} type="date" allowEditing={false} visible={false} />
                        <ColumnDirective field="CreatedBy" headerText="Created By" width="160" allowEditing={false} visible={false} />
                        <ColumnDirective field="Modified" headerText="Modified" width="160" format={formatAmPm} type="date" allowEditing={false} visible={false} />
                        <ColumnDirective field="ModifiedBy" headerText="ModifiedBy" width="160" allowEditing={false} visible={false} />
                    </ColumnsDirective>
                    <Inject
                        services={[
                            Page,
                ...
                <GridComponent
                    dataSource={dataFromCounselAddressApi}
                ...
                <GridComponent
                    dataSource={dataFromCounselAttorneyApi}
                    allowSelection={false}
                    allowPaging={true}
                    ref={(grid) => (gridInstance = grid)}
                    pageSettings={pageOptions}
