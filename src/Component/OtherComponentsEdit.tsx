...
function OtherComponentsEdit(props : any) {
  const { rowData } = props
  const [localStateContent, setlocalStateContent] = useState<any>({
    PostureId: rowData.PostureId ?? "",
    DateReleaseReceived: rowData.DateReleaseReceived ?? "",
    DealerName: rowData?.DealerName ?? "",
    CivilPenaltiesEstimate: rowData?.CivilPenaltiesEstimate ?? "",
    VehicleYear: rowData?.VehicleYear ?? "",
    DaysOutOfService: rowData?.DaysOutOfService ?? "",
    AttorneysFeesandCostsRequested: rowData.AttorneysFeesandCostsRequested ?? false,
    TenderOfDefenseFromDealerFlag: rowData.TenderOfDefenseFromDealerFlag ?? false,
    CivilPenaltiesFlag: rowData.CivilPenaltiesFlag ?? false,
  });

  useEffect(() => {
    localStorage.setItem("localOtherComponentContentEdit", JSON.stringify(localStateContent));
  }, [localStateContent]);
...
export default memo(OtherComponentsEdit)